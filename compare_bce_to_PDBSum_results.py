...
os.chdir('/home/sayantoni/Desktop/test_complex')

#-- READ MMCIF FILE

parser = MMCIFParser()
structure = parser.get_structure("2a1j", "2a1j.cif")
file_name = "2a1j.cif"
...
pick_res_id = list(pick_CA['residue_id'])
tem1["residue"] = pick
tem1['atom_assym'] = pick_assym
tem1['residue_id'] = pick_res_id


#--- FETCH PDB-COMPATIBLE RESIDUE IDs
from biopandas.pdb import PandasPdb
ppdb = PandasPdb().fetch_pdb('2a1j')
pdb_df = ppdb.df['ATOM']
pick_CA_pdb = pdb_df[pdb_df["atom_name"] == 'CA']
pick_pdb_res_no = list(pick_CA_pdb['residue_number'])

#--- APPEND COLUMN TO BETWEENNESS DATA
tem1['residue_number'] = pick_pdb_res_no


#--- FIND BETWEENNESS AND SORT 
sorted_by_betweennes = tem1.sort_values(["betweenness"], ascending = [False])
sorted_by_betweennes['rank_by_betweenness'] = list(range(1,len(pick)+1))
sorted_by_betweennes.to_excel("sorted_by_betweennes.xlsx")


#--- FIND OUT INTERFACE RESIDUES FROM PDBSUM AND COMPARE
import requests
PDB_ID = "2a1j"
...
with open('PDBSum_results.txt', 'w') as f:
    f.write(k)
