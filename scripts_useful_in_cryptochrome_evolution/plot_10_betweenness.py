...
def top_10_percent_betweenness(file):
    # Calculate node and edge betweenness
    node_df, edge_df = calculate_betweenness(file)
    
    # Calculate the threshold for top 10% node betweenness
    node_threshold = node_df['node_betweenness'].quantile(0.90)
    
    # Filter the nodes that are in the top 10%
    top_10_percent_nodes = node_df[node_df['node_betweenness'] >= node_threshold]
    
    # Return the dataframe with required columns
    return top_10_percent_nodes[['residue_id', 'residue_name', 'node_betweenness']]

# # Example usage:
# for file in cif_files:
#     protein_name = os.path.basename(file).split('.')[0]
#     top_nodes_df = top_10_percent_betweenness(file)
    
#     save_path = f"Top10PercentNodeBetweenness_{protein_name}.csv"
#     top_nodes_df.to_csv(save_path, index=False)
...
def plot_10_betweenness(file):
    cutoff_list = _network(file)
    graph = nx.from_pandas_edgelist(cutoff_list, 'source', 'target', ['distance'])

    # Calculate betweenness centrality
    centrality = nx.betweenness_centrality(graph)
    
    # Sort nodes by centrality and select the top 10%
    sorted_centralities = sorted(centrality.items(), key=lambda x: x[1], reverse=True)
    top_10_percent = int(0.10 * len(graph.nodes()))
    top_nodes = [node[0] for node in sorted_centralities[:top_10_percent]]
    
    # Extract 3D coordinates for residues from fetch function
    df = fetch(file)
    pos = {}
    for index, row in df.iterrows():
        atom_name = "ATM" + row['ca_atom_number']
        pos[atom_name] = (float(row['x_coord']), float(row['y_coord']), float(row['z_coord']))

    # Plot the protein in 3D
    ...
    for edge in graph.edges():
        ...
        ax.plot(x_values, y_values, z_values, color='dodgerblue', linewidth=0.5)  # Reduced line width with linewidth=0.5

    for node in graph.nodes():
        color = 'purple' if node in top_nodes else 'darkblue'  
        ax.scatter(*pos[node], color=color, s=15)  

    # Remove grids, axes and spines
    ax.grid(False)
    ax.axis('off')
    ...
    # Extract the filename (without its path and extension) to use it as a label
    label = os.path.splitext(os.path.basename(file))[0]
    ax.set_title(label)  # Set the label as title
    
    # Save the plot with the respective label (filename without extension)
    plt.savefig(f"{label}_network_plot.png", format="png")
    plt.show()

# Process and save each file
for file_path in cif_files:
    plot_10_betweenness(file_path)
